#include<pthread.h>   //create multiple threads for concurrent process flow.
#include<iostream>
#include<time.h>
#include<unistd.h>
#include<iomanip>
...
void* ATB(void* id)
{
        ...
        if(::atbcnt==1)                    // if the 1st villager close the othe             r bridge entry
                pthread_mutex_trylock(&bta);
        if(::atbcyc!=STARVCNT)              //if count is less than starvation u             nlock our bridge
        ...
        ::onatb++;
        while(::onatb>1); //a villager may have to travel behind another village             r
        ...
        pthread_mutex_unlock(&mutex);
        sleep(2);
        if(!pthread_mutex_trylock(&bta))      //if opposite village bridge is lo             cked
        {
                pthread_mutex_unlock(&atb);       //open both the village bridge             s
                pthread_mutex_unlock(&bta);
        }
        pthread_exit(NULL);
}
void* BTA(void* id)
{
        ...
        pthread_mutex_lock(&mutex1);
        ::onbta++;
        while(::onbta>1); //a villager may have to travel behind another village             r
        pthread_mutex_unlock(&mutex1);
        sleep((rand()%5)+2);
